...
class TransformHeadersActor(phaseId : String) extends AbstractAfthemActor(phaseId) {
  ...
  def perform(msg : BaseMessage, wrapper : HttpWrapper): Unit ={
    val headersToRemove = evaluateConfigHeaders(msg,getPhase(msg).getConfigListEvalNameValue("remove"))
    removeHeaders(wrapper,headersToRemove)
    val headersToAdd = evaluateConfigHeaders(msg,getPhase(msg).getConfigListEvalNameValue("add"))
    addHeaders(wrapper,headersToAdd)
    val headersToSet = evaluateConfigHeaders(msg,getPhase(msg).getConfigListEvalNameValue("set"))
